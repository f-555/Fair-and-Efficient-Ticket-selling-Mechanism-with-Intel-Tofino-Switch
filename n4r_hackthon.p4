/*******************************************************************************
 * P4 SOURCE CODE FOR HACKTHON 2022
...
#define ID_WIDTH 32 //user id length
#define BF_INDEX_HASH_LEN 14 //width of bloom filter index, maxium num is 15
#define BF_SIZE 65536 //size of bloom filter, 2 to the (BF_INDEX_HASH_LEN)th power
#define MAX_TICKET 1000 //number of tickets

#define BF_INDEX_WIDTH 16 //fixed in this vision, width of bloom filter, donot change this, change BF_INDEX_HASH_LEN to change BF_SIZE
...
struct bloomfilter_chain_metadata_t {
    bit<8> nextnode_flag;
    bit<8> valid_user_flag1;//flags for bloom filter1
    bit<8> valid_user_flag2;
    bit<8> valid_user_flag3;
    bit<8> nextnode_flag2;

    bit<8> valid_user_flag2_1;//flags for bloom filter2
    bit<8> valid_user_flag2_2;
    bit<8> valid_user_flag2_3;
    bit<8> nextnode_flag3;

    bit<8> valid_user_flag3_1;//flags for bloom filter2
...
//bloom filter 3
    ...
    // HashAlgorithm: IDENTITY, RANDOM, CRC64, CRC32
    ...
    CRCPolynomial<bit<32>>(32w0xad0424f3, // polynomial
                           true,          // reversed
                           false,         // use msb?
                           false,         // extended?
    ...
    CRCPolynomial<bit<32>>(32w0x973afb51, // polynomial
                           false,          // reversed
                           false,         // use msb?
                           false,         // extended?
                           32w0xFFFFFFFF, // initial shift register value
    ...
    CRCPolynomial<bit<32>>(32w0xad0454f3, // polynomial
                           true,          // reversed
                           false,         // use msb?
                           false,         // extended?
                           32w0xFFFFFFFF, // initial shift register value
    ...
    action compute_hash11(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field11 = hash11.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash12(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field12 = hash12.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash21(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field21 = hash21.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash22(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field22 = hash22.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash31(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field31 = hash31.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash32(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field32 = hash32.get(USER_KEY);// & 0x000003ff;
    }

    action compute_hash11_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field11 = hash13.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash12_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field12 = hash14.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash21_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field21 = hash23.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash22_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field22 = hash24.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash31_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field31 = hash33.get(USER_KEY);// & 0x000003ff;
    }
    action compute_hash32_r(){
        // output BF_INDEX_WIDTH bit hash result to index 
        ig_md.md_fc.hash_field32 = hash34.get(USER_KEY);// & 0x000003ff;
    }
    ...
    apply {
        ...
        if(ig_md.md_fc.bf_result11 == 0 || ig_md.md_fc.bf_result12 == 0){ //new user
            ig_md.md_fc.valid_user_flag1 = 1;//new user, no conflict
        }
        else if(ig_md.md_fc.bf_result11 != hdr.ticket.usr_id && ig_md.md_fc.bf_result12 != hdr.ticket.usr_id){ //new user conflict
            ig_md.md_fc.nextnode_flag = 1;
            ig_md.md_fc.valid_user_flag2 = 1; //new user, conflict
        }
        if(ig_md.md_fc.bf_result11 == hdr.ticket.usr_id || ig_md.md_fc.bf_result12 == hdr.ticket.usr_id){ //old user
            ig_md.md_fc.valid_user_flag3 = 1; //old user
        ...
        if(ig_md.md_fc.nextnode_flag == 1){
            ...
            if(ig_md.md_fc.bf_result21 == 0 || ig_md.md_fc.bf_result22 == 0){ //new user
                ig_md.md_fc.valid_user_flag2_1 = 1;//new user, no conflict
            }
            else if(ig_md.md_fc.bf_result21 != hdr.ticket.usr_id && ig_md.md_fc.bf_result22 != hdr.ticket.usr_id){ //new user conflict
                ig_md.md_fc.nextnode_flag2 = 1;
                ig_md.md_fc.valid_user_flag2_2 = 1; //new user, conflict
            }
            if(ig_md.md_fc.bf_result21 == hdr.ticket.usr_id || ig_md.md_fc.bf_result22 == hdr.ticket.usr_id){ //bypass
                ig_md.md_fc.valid_user_flag2_3 = 1; //old user
                hdr.ticket.opcode=0b01000000;//ddos?
            }
        }
        ...
        if(ig_md.md_fc.nextnode_flag2 == 1){
            ...
            if(ig_md.md_fc.bf_result31 == 0 || ig_md.md_fc.bf_result32 == 0){ //new user
                ig_md.md_fc.valid_user_flag3_1 = 1;//new user, no conflict
            }
            else if(ig_md.md_fc.bf_result31 != hdr.ticket.usr_id && ig_md.md_fc.bf_result32 != hdr.ticket.usr_id){ //new user conflict
                ig_md.md_fc.nextnode_flag3 = 1;
                ig_md.md_fc.valid_user_flag3_2 = 1; //new user, conflict
                ig_dprsr_md.resubmit_type = 3w1; //resubmit
            }
            if(ig_md.md_fc.bf_result31 == hdr.ticket.usr_id || ig_md.md_fc.bf_result32 == hdr.ticket.usr_id){ //bypass
                ig_md.md_fc.valid_user_flag3_3 = 1; //old user
                hdr.ticket.opcode=0b01000000;//ddos?
            }
        }
        ...
        if(hdr.ticket.isValid()){
            if(ig_md.md_fc.valid_user_flag2_1 == 1 || ig_md.md_fc.valid_user_flag1 == 1 || ig_md.md_fc.valid_user_flag3_1==1){
                ...
                //hdr.ticket.counter = flow_num_read_op.execute(hdr.ticket.ticket_type);
